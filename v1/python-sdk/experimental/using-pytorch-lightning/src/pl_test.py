...
def test():
    try:
        print("running nebula support pytorch_lightning test")
        os.environ["DLTS_NUM_WORKER"] = "1"
        os.environ["FC_TASK_INDEX"] = "0"
        os.environ["FC_TASKROLE_NAME"] = "worker"
        os.environ["DLTS_JOB_ID"] = "test" + str(int(time.time() * 1000))
        ...
        trainer = pl.Trainer(
        gpus=0, strategy="ddp", max_epochs=20, plugins=[tn.NebulaCheckpointIO()], callbacks=[nebula_callback])
        
        trainer.fit(autoencoder, DataLoader(train), DataLoader(val))
        for parent, dirnames, filenames in os.walk(persisted_path):
        ...
        print(js)
        if ckpts["status_code"] != 200 or ckpts["tags"] is None:
            print("[Error] failed to list_checkpoints")
            return False

        latest_ckpt_path = tn.get_latest_checkpoint_path("checkpoint", persisted_path)
        print("\n*********** get latest checkpoints path **********\n")
        if latest_ckpt_path == None:
            print("[Error] failed to get_latest_checkpoint_path")
            return False
...
if __name__ == "__main__":
    ...
    result = test()
    file_name = os.environ["HOME"] + "/tmp/result3.txt"
